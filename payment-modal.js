 ...
 * Версия: 5.0.0 - Миграция на Render.com
 * Дата: Январь 2025
 * 
 * Обновления v5.0.0:
 * - Миграция с Netlify на Render.com
 * - Новый URL: /api/send-to-n8n для Render serverless функций
...
console.log('📄 payment-modal.js v5.0.0 загружен - Render.com + n8n через Render API + T-Bank виджет');
...
async function createTBankPayment() {
    ...
    try {
        console.log('📤 Отправляем данные в n8n через Render API...');
        const n8nResponse = await fetch('/api/send-to-n8n', {
            method: 'POST',
            headers: {
